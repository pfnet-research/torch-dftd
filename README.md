...
`TorchDFTD4Calculator` / `DFTD4Module` implement DFT-D4 (Caldeweyher et al., JCP 150, 154122 (2019)):
BJ damping, EEQ partial charges (Ewald summation under periodic boundary conditions), CN/charge-weighted
reference polarizabilities, and the ATM three-body term. Results agree with the reference `dftd4` Fortran
library to ~1e-6 eV (energies), ~1e-7 e (charges) and ~1e-6 eV/Å (forces) for molecules and crystals.

```python
from torch_dftd.torch_dftd4_calculator import TorchDFTD4Calculator
calc = TorchDFTD4Calculator(xc="pbe", device="cuda:0")         # standard D4: ATM on, dftd4 default cutoffs
calc = TorchDFTD4Calculator(xc="pbe", abc_cutoff=20 * Bohr)      # cheaper three-body cutoff (~0.1 kJ/mol)
```

Reference data (`torch_dftd/nn/params/dftd4_params.npz`, `dftd4_xc_params.py`) are generated by
`tools/gen_dftd4_params.py` from the `tad-dftd4` and `dftd4` packages. Only fully periodic or
non-periodic systems are supported (no slabs/wires).
