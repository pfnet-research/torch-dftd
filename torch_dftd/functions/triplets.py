...
def _calc_triplets_core(
    ...
    triplet_node_index_list: List[List[int]] = []  # (n_triplet_edges, 3)
    edge_jk_list: List[List[int]] = []  # (n_triplet_edges, 2) represents j and k indices
    multiplicity_list: List[float] = []  # (n_triplet_edges) represents multiplicity
    batch_triplets_list: List[int] = []  # (n_triplet_edges) represents batch index for triplets
    ...
    for i in range(len(unique)):
        ...
        for j in range(_n_edges - 1):
            for k in range(j + 1, _n_edges):
                _dst0: int = _dst[j]  # _dst0 maybe swapped with _dst1, need to reset here.
                _dst1: int = _dst[k]
