    ...
    for cname,cut in cutsets.items():
        for atm in (False,True):
            ...
            for dev in ("cpu","cuda"):
                ...
                try:
                    calc=TorchDFTD4Calculator(xc="pbe",device=dev,dtype=torch.float32,abc=atm,cutoff=cut[0]*Bohr,abc_cutoff=cut[1]*Bohr,cnthr=cut[2]*Bohr)
                    b=a.copy(); b.calc=calc
                    def E():
                        b.rattle(1e-6, seed=int(time.time()*1e6)%2**31); e=b.get_potential_energy()
                        if dev=="cuda": torch.cuda.synchronize()
                        return e
                    def EF():
                        b.rattle(1e-6, seed=int(time.time()*1e6)%2**31); f=b.get_forces()
                        if dev=="cuda": torch.cuda.synchronize()
                    row[f"torch_{dev}_E_ms"]=round(1e3*med(E),1); row[f"torch_{dev}_EF_ms"]=round(1e3*med(EF,2),1)
                    t=time.perf_counter(); d=calc._preprocess_atoms(b); row[f"torch_{dev}_edges_ms"]=round(1e3*(time.perf_counter()-t),1); row["n_edges"]=int(d["edge_index"].shape[1])
                    row[f"E_torch_{dev}"]=round(float(b.get_potential_energy()),6)
                    if dev=="cuda": row["gpu_GB"]=round(torch.cuda.max_memory_allocated()/2**30,2); torch.cuda.reset_peak_memory_stats()
