...
for name, a in [
    ...
    for atm in (False, True):
        try:
            ...
            e = b.get_potential_energy()
            print(
                f"{name:18s} N={len(a):5d} atm={int(atm)} GPU E {1e3*tE:7.1f} ms  E+F {1e3*tEF:7.1f} ms  peak {torch.cuda.max_memory_allocated()/2**30:5.2f} GB  "
                + (f"dE vs Fortran(atm) {e-Ef:+.1e} eV" if atm else ""),
                flush=True,
            )
        except Exception as ex:
            print(
                f"{name:18s} N={len(a):5d} atm={int(atm)} FAIL {str(ex).splitlines()[0][:100]}",
                flush=True,
            )
