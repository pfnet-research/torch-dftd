...
from dftd4.interface import (
    DampingParam,
    DispersionModel,
)  # import BEFORE torch (symbol clash otherwise)
import torch
...
def fortran(atoms, atm, cutoffs=None, charge=0.0):
    ...
    m = DispersionModel(
        atoms.numbers, atoms.positions / Bohr, charge=charge, lattice=lat, periodic=per
    )
    if cutoffs:
    ...
    return dict(
        E=res["energy"] * Hartree,
        F=-res["gradient"] * Hartree / Bohr,
        q=props["partial charges"],
        cn=props["coordination numbers"],
    )
...
def run(name, atoms, atm, cut_bohr=(60, 40, 30), device="cpu", dtype=torch.float64, charge=0.0):
    ...
    calc = TorchDFTD4Calculator(
        ...
        dtype=dtype,
        abc=atm,
        cutoff=cut_bohr[0] * Bohr,
        abc_cutoff=cut_bohr[1] * Bohr,
        cnthr=cut_bohr[2] * Bohr,
        charge=charge,
    )
    a = atoms.copy()
    a.calc = calc
    ...
    print(
        f"{name:28s} N={len(a):4d} atm={int(atm)} E_torch={E:12.6f} E_fortran={ref['E']:12.6f} dE={E-ref['E']:+.2e} eV | max|dq|={np.abs(q-ref['q']).max():.2e} | max|dF|={np.abs(F-ref['F']).max():.2e} eV/A | {dt*1e3:.0f} ms",
        flush=True,
    )
    return E, ref
...
if __name__ == "__main__":
    ...
    m = molecule("NH3")
    m.pbc = False
    run("NH4+ (NH3 geom, q=+1)", m, True, charge=1.0)
